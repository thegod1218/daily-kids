...
function ChildTabBar() {
  ...
  return (
    <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #E6DDD3', display: 'flex', justifyContent: 'space-around', padding: '10px 0 calc(10px + env(safe-area-inset-bottom))', zIndex: 100 }}>
      {tabs.map(t => (
...
export default function HomePage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F3EE', paddingBottom: 90, fontFamily: 'sans-serif' }}>
      ...
      <div style={{ padding: '16px 20px 0' }}>
        ...
        {/* 오늘의 목표 */}
        <div style={{ background: '#FFFDF9', borderRadius: 22, border: '1px solid #E6DDD3', padding: 18, marginBottom: 12, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            ...
            <button onClick={() => nav('/child/missions')} style={{ padding: '11px 18px', borderRadius: 999, fontSize: 14, border: 'none', background: '#F3F0ED', color: '#111', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>시작</button>
          </div>
          <ProgBar value={total > 0 ? (done / total) * 100 : 0} style={{ marginBottom: 10 }} />
          <div style={{ fontSize: 14, color: '#7C746D', lineHeight: 1.6 }}>하나씩 천천히 해보면 돼요.</div>
          <div style={{ marginTop: 16, display: 'flex', justifyContent: 'center' }}>
            <span style={{ fontSize: 80 }}>{renderChar(80)}</span>
          </div>
        </div>
        ...
        <div style={{ background: '#FFFDF9', borderRadius: 22, border: '1px solid #E6DDD3', padding: 18, marginBottom: 12 }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: '#2C2926', marginBottom: 4 }}>오늘의 마음</div>
          <div style={{ fontSize: 13, color: '#7C746D', marginBottom: 14 }}>하나만 골라봐요.</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
            {MOODS.map(({ type, e, label }) => (
              <button key={type} onClick={() => handleMood(type)} style={{ flex: 1, minHeight: 76, borderRadius: 18, cursor: 'pointer', fontFamily: 'inherit', border: todayMood?.type === type ? '1.5px solid #D28A6A' : '1px solid #E6DDD3', background: todayMood?.type === type ? '#F7E6DD' : '#FFFDF9', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                <span style={{ fontSize: 24 }}>{e}</span>
                <span style={{ fontSize: 11, color: '#7C746D', fontWeight: 700 }}>{label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: '#FFFDF9', borderRadius: 22, border: '1px solid #E6DDD3', padding: 18, marginBottom: 8 }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: '#2C2926', marginBottom: 4 }}>짧은 일기</div>
          ...
          <textarea
            ...
            style={{ width: '100%', minHeight: 92, resize: 'none', fontSize: 14, lineHeight: 1.6, paddingTop: 12, border: '1px solid #E6DDD3', borderRadius: 14, padding: 14, outline: 'none', fontFamily: 'inherit', background: '#FFFDF9' }}
          />
